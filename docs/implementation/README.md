...
Aircraft Page, Cockpit Explorer, and Aircraft Systems remain unavailable until their existing Phase 5–7 work items are implemented. Their canonical aircraft- and implementation-scoped routes are defined by [ADR-0013](../decisions/ADR-0013-aircraft-and-implementation-route-scopes.md); the shell keeps them as non-link coming-soon states until those real routes exist. Guide Mode retains its separate Focus Mode chrome. Its current routes require the ADR-0013 backward-compatible migration before canonical content publication. Verified real cockpit media remains deferred and does not block the shell.
